...
export default async function NocturnaPage() {
  const assets = await db.query.photos.findMany({
    where: eq(photos.categoryId, 'nocturna'),
          ...
          {assets.map((asset, index) => (
            ...
            >
               {asset.type === 'video' ? (
                 <VideoEmbed 
                   url={asset.url} 
                   title={asset.title || 'Video nocturna'} 
                 />
               ) : (
                 <LightboxImage 
                   imgSrc={asset.url} 
                   alt={asset.title || `Fotografía nocturna ${index + 1}`}
                 />
               )}
